...
result_folder <- "/beegfs/scratch/ric.cosr/pedrini.edoardo/test/test_cellranger/test100/test_pbmc_token"
# result_folder <- snakemake@input$result_folder

# define the sample id
# folder = directory(config["out_location"] + "cellranger/merged/{sample_name}")
in_id_sample <- "test_pbmc_token"
...
# read in the cell annotation
LUT_cells <- read_csv("/beegfs/scratch/ric.cosr/pedrini.edoardo/test/test_cellranger/test100/test_pbmc_token/outs/cell_types/cell_types.csv")
# test_markers <- read_csv("/beegfs/scratch/ric.cosr/pedrini.edoardo/test/test_cellranger/test100/test_pbmc_token/outs/cell_types/cell_annotation_differential_expression.csv")
...
out_id_object <-  "out/object/01_test_pbmc_token_preQC.rds"
out_id_meta <- "out/table/01_test_pbmc_token_meta_preQC.tsv"
...
# add the automatic annotation result in the object
meta_full <- datasc@meta.data %>%
  rownames_to_column("barcode") %>%
  left_join(LUT_cells,by = "barcode") %>%
  column_to_rownames("barcode")

# update the metadata
datasc@meta.data <- meta_full
