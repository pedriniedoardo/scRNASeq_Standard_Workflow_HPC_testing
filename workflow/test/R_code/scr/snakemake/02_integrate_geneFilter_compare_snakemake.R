...
spe_filter <- snakemake@input$spe_filter
message("input filter: ", spe_filter)

# spe_all <- "out/object/test_human_default/01_connect_5k_pbmc_NGSC3_ch1_gex_all.rds"
spe_all    <- snakemake@input$spe_all
message("input all: ", spe_all)
...
nm  <- snakemake@params$nm
message("base name file: ", nm)

# define the output
# p03_out <- paste0("out/plot/04_connect_5k_pbmc_NGSC3_ch1_gex_UMAP_",res,".pdf")
p03_out <- snakemake@output$plot_umap
message("plot UMAP: ", p03_out)

# ht02_out <- paste0("out/plot/04_connect_5k_pbmc_NGSC3_ch1_gex_Jaccard_",res,".pdf")
ht02_out <- snakemake@output$plot_jaccard
message("plot jaccard: ", ht02_out)

# tab01_all <- paste0("out/table/04_connect_5k_pbmc_NGSC3_ch1_gex_markersAll_",res,".tsv")
tab01_all <- snakemake@output$tab_markers_all
message("tab marker all: ", tab01_all)

# tab01_filter <- paste0("out/table/04_connect_5k_pbmc_NGSC3_ch1_gex_markersFilter_",res,".tsv")
tab01_filter <- snakemake@output$tab_markers_filt
message("tab marker filter: ", tab01_filter)

# p04_out <- "out/plot/04_connect_5k_pbmc_NGSC3_ch1_gex_UpsetPlot.pdf"
p04_out <- snakemake@output$plot_upset
message("plot upset: ", p04_out)

# p05_out <- "out/plot/04_connect_5k_pbmc_NGSC3_ch1_gex_BoxGeneExp.pdf"
p05_out <- snakemake@output$plot_exp
message("plot boxplot avg gene exp: ", p05_out)

# p06_out <- paste0("out/plot/04_connect_5k_pbmc_NGSC3_ch1_gex_BoxPropHVG_",res,".pdf")
p06_out <- snakemake@output$plot_prop_hvg
message("plot boxplot prop hvg: ", p06_out)

# custom functions --------------------------------------------------------
# define the jaccard score function
jaccard <- function(a, b) {
  # there should be no replicates in the sequence of the entries
  # nevertheless force them to be unique
  a <- unique(a)
  b <- unique(b)
...
b <- c('car', 'chips', 'bird', 'salt',"car","car")
